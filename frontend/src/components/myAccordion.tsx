...
interface AccordionItemProps {
    ...
    isOpen: boolean;
    onClick: () => void;
    colorClassTitle?: string,
    colorClassDescription?: string
}

export const AccordionItem: React.FC<AccordionItemProps> = ({ title, content, isOpen, onClick, colorClassTitle, colorClassDescription }) => {
    ...
    return (
        <div className="relative  mb-3">
            <h3 className=" font-bold mb-0">
                ...
                >
                    <p className='l'><span className={`text-lg sm:text-xl  md:text-2xl ${colorClassTitle}`}>{title}</span></p>
            ...
            >
                <div ref={contentRef} className={`p-4  rounded-lg ${colorClassDescription} text-base md:text-lg leading-normal font-medium`}>
                    {content}
                </div>
            </div>
        </div>
